...
function VueDevis({chantiers,salaries,statut,entreprise}){
  ...
  function calcDocTotal(doc){const ht=(doc.lignes||[]).reduce((a,l)=>a+l.qte*l.prixUnitHT,0);const tv=(doc.lignes||[]).reduce((a,l)=>a+l.qte*l.prixUnitHT*((l.tva||20)/100),0);return{ht:+ht.toFixed(2),tv:+tv.toFixed(2),ttc:+(ht+tv).toFixed(2)};}(doc){const ll=doc.lignes||[];const ht=ll.reduce((a,l)=>a+(+l.qte||0)*(+l.prixUnitHT||0),0);const tv=ht*0.2;return{ht:+ht.toFixed(2),tv:+tv.toFixed(2),ttc:+(ht+tv).toFixed(2)};}(doc){var ll=doc.lignes&&doc.lignes.length?doc.lignes:(doc.tranches||[]).reduce(function(a,t){return a.concat(t.lignes||[]);},[]); var ht=+ll.reduce(function(a,l){return a+(l.totalHT?+l.totalHT:+(l.qte||0)*(+(l.prixUnitHT||l.puHT)||0));},0).toFixed(2);var tva=+(doc.tauxTVA||20);var tv=+(ht*tva/100).toFixed(2);return{ht:ht,tv:tv,ttc:+(ht+tv).toFixed(2)};}(doc){var ll=doc.lignes&&doc.lignes.length?doc.lignes:[];if(!ll.length&&doc.tranches){for(var i=0;i<doc.tranches.length;i++){if(doc.tranches[i].lignes){ll=ll.concat(doc.tranches[i].lignes);}}}var ht=0;for(var j=0;j<ll.length;j++){var l=ll[j];ht+=l.totalHT?+l.totalHT:+(l.qte||0)*(+(l.prixUnitHT||l.puHT)||0);}ht=+ht.toFixed(2);var tva=+(doc.tauxTVA||20);var tv=+(ht*tva/100).toFixed(2);return{ht:ht,tv:tv,ttc:+(ht+tv).toFixed(2)};}(doc){const lignes=(doc.lignes||[]).length?(doc.lignes||[]):(doc.tranches||[]).reduce((a,t)=>a.concat(t.lignes||[]),[]);const ht=+lignes.reduce((a,l)=>a+(+(l.totalHT)||+(l.qte||0)*(+(l.prixUnitHT||l.puHT)||0)),0).toFixed(2);const tva=+(doc.tauxTVA||20);const tv=+(ht*tva/100).toFixed(2);return{ht,tv,ttc:+(ht+tv).toFixed(2)};}(doc){const lignes=doc.lignes?.length?doc.lignes:(doc.tranches||[]).flatMap(t=>t.lignes||[]);const ht=+lignes.reduce((a,l)=>a+(l.totalHT??+(l.qte*(l.prixUnitHT||l.puHT||0)).toFixed(2)),0).toFixed(2);const tva=doc.tauxTVA||20;const tv=+(ht*tva/100).toFixed(2);return{ht,tv,ttc:+(ht+tv).toFixed(2)};}(doc){const ht=(doc.lignes||[]).reduce((a,l)=>a+l.qte*l.prixUnitHT,0);const tv=(doc.lignes||[]).reduce((a,l)=>a+l.qte*l.prixUnitHT*(l.tva/100),0);return{ht,tva:tv,ttc:ht+tv};}
  return(
    <div>
